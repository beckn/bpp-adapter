...
export const status = async (filter: any) => {
  if (filter.context.domain.trim() === "online-dispute-resolution:0.1.0") {
    filter.context["action"] = "on_status";
            ...
            order:{
                ...
                provider: {
                    ...
                    descriptor: {
                      ...
                      images: [
                        {
                          url: res?.attributes?.provider?.data?.attributes?.logo?.data?.attributes?.url
                            ? res?.attributes?.provider?.data?.attributes?.logo?.data?.attributes?.url
                            : "http://abc.com/image.jpg",
                        },
                      ],
                    },
                    ...(res?.attributes?.provider?.data?.attributes?.category_ids?.data && res?.attributes?.provider?.data?.attributes?.category_ids.data.length > 0
                        ? {
                            categories: res?.attributes?.provider?.data?.attributes?.category_ids.data.map((cat:any) => {
                  ...
                  items: [
                    {
                      ...
                      category_ids: res?.attributes?.cat_attr_tag_relations?.data?.map((e:any) => e.attributes.taxanomy_id)
                    },
                  ],
                  quote: {
                    price: {
